...
# Replace this with the name of the network you see with ifconfig command in the terminal
network_name = "eth0"
...
def changeLink(n1, n2, state):
    if (state == "True"):
        subprocess.call(shlex.split(
            "sudo iptables -I OUTPUT -s 10.1.1.%d -d 10.1.1.%d -j ACCEPT" % (n1, n2)))
        subprocess.call(shlex.split(
            "sudo iptables -I OUTPUT -s 10.1.1.%d -d 10.1.1.%d -j ACCEPT" % (n2, n1)))
    else:
        subprocess.call(shlex.split(
            "sudo iptables -D OUTPUT -s 10.1.1.%d -d 10.1.1.%d -j ACCEPT" % (n1, n2)))
        subprocess.call(shlex.split(
            "sudo iptables -D OUTPUT -s 10.1.1.%d -d 10.1.1.%d -j ACCEPT" % (n2, n1)))
...
def run_test(i):
    ...
    subprocess.call(["perl", maketopology_file,
                    cur_directory+"topology.txt", network_name])
    ...
    for nodeID in nodes:
        ...
        process = Popen([program_name, cur_node, cur_cost, directory_results +
                        "test"+str(i)+"_log"+str(nodeID)], stdout=sys.stdout, stderr=sys.stdout)
